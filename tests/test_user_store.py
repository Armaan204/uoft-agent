...
class TestGetOrCreateUser:
    ...
    def test_empty_google_id_raises(self):
        from api.auth.user_store import get_or_create_user, UserStoreError
...
class TestSaveQuercusToken:
    def test_inserts_new_token_when_none_exists(self):
        from api.auth.user_store import save_quercus_token
        mock_sb = _mock_sb()
        # No existing token
        mock_sb.table.return_value.select.return_value.eq.return_value.limit.return_value \
    ...
    def test_raises_on_empty_user_id(self):
        from api.auth.user_store import save_quercus_token, UserStoreError
        with pytest.raises(UserStoreError):
...
class TestGetQuercusToken:
    def test_returns_none_for_empty_user_id(self):
        from api.auth.user_store import get_quercus_token
    ...
    def test_decrypts_and_returns_token(self):
        ...
        from api.auth.user_store import save_quercus_token, get_quercus_token
        ...
        with patch("api.auth.user_store.get_supabase_client", return_value=mock_sb_save):
        ...
        with patch("api.auth.user_store.get_supabase_client", return_value=mock_sb_get):
            result = get_quercus_token("u1")

        assert result == plaintext

    def test_raises_on_corrupted_encrypted_token(self):
        from api.auth.user_store import get_quercus_token, UserStoreError
        mock_sb = _mock_sb()
...
class TestDeleteQuercusToken:
    def test_calls_delete_on_supabase(self):
        from api.auth.user_store import delete_quercus_token
        mock_sb = _mock_sb()
    ...
    def test_raises_user_store_error_on_exception(self):
        from api.auth.user_store import delete_quercus_token, UserStoreError
        mock_sb = _mock_sb()
...
class TestUserStoreHelpers:
    def test_secret_or_env_reads_value(self, monkeypatch):
        from api.auth.user_store import _secret_or_env
    ...
    def test_get_supabase_client_requires_config(self, monkeypatch):
        from api.auth.user_store import get_supabase_client, UserStoreError
    ...
    def test_get_supabase_client_uses_create_client(self, monkeypatch):
        ...
        with patch("api.auth.user_store.create_client", return_value=fake) as mock_create:

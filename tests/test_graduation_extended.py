...
class TestHtmlToText:
    def test_strips_html_tags(self):
        from api.integrations.graduation_service import _html_to_text
...
class TestHasCourseRequirements:
    def test_returns_false_for_few_codes(self):
        from api.integrations.graduation_service import _has_course_requirements
...
class TestSlugifyUtsc:
    def test_basic_slug(self):
        from api.integrations.graduation_service import _slugify_utsc
...
class TestGenerateUtscSlugVariants:
    def test_returns_base_slug(self):
        from api.integrations.graduation_service import _generate_utsc_slug_variants
...
class TestCollectRequiredCourses:
    def test_collects_required_and_list_courses(self):
        from api.integrations.graduation_service import collect_required_courses
        reqs = {
            "groups": [{
                "items": [
...
class TestIsValidProgramPage:
    def test_accepts_page_with_course_codes(self):
        from api.integrations.graduation_service import _is_valid_program_page
        page = {
...
class TestLoadCache:
    def test_returns_requirements_on_hit(self):
        from api.integrations.graduation_service import _load_cache
        ...
        with patch("api.integrations.graduation_service._db", return_value=mock_db):
...
class TestSaveCache:
    def test_calls_upsert(self):
        from api.integrations.graduation_service import _save_cache
        mock_db = MagicMock()
...
class TestFetchPage:
    def test_returns_dict_on_200(self):
        from api.integrations.graduation_service import _fetch_page
        mock_resp = MagicMock(status_code=200, text="<html>CSCA08H3 content</html>")
        with patch("api.integrations.graduation_service.requests.get", return_value=mock_resp):
    ...
    def test_returns_none_on_exception(self):
        from api.integrations.graduation_service import _fetch_page
        with patch("api.integrations.graduation_service.requests.get", side_effect=Exception("timeout")):
            result = _fetch_page("http://example.com")
        assert result is None
...
class TestExtractWithLlm:
    ...
    def test_returns_parsed_requirements(self):
        from api.integrations.graduation_service import _extract_with_llm
        ...
        with patch("api.integrations.graduation_service._llm", return_value=mock_llm):
...
class TestGetProgramRequirements:
    def test_cache_hit_returns_cached(self):
        ...
        with patch("api.integrations.graduation_service._load_cache", return_value=cached):
...
class TestSaveFailed:
    def test_calls_upsert_with_failed_status(self):
        from api.integrations.graduation_service import _save_failed
        mock_db = MagicMock()
...
class TestClearCache:
    def test_calls_delete_on_table(self):
        from api.integrations.graduation_service import clear_cache
        mock_db = MagicMock()
...
class TestProgramUrlKeywords:
    def test_filters_stop_words(self):
        from api.integrations.graduation_service import _program_url_keywords
...
class TestDdgSearchUrls:
    def test_returns_matching_urls(self):
        from api.integrations.graduation_service import _ddg_search_urls
        import urllib.parse
...
class TestStripCoopQualifier:
    def test_removes_co_operative(self):
        from api.integrations.graduation_service import _strip_coop_qualifier
...
class TestDiscoverUrlViaSlug:
    def test_returns_page_on_slug_match(self):
        from api.integrations.graduation_service import _discover_url_via_slug
        fake_page = {
        ...
        with patch("api.integrations.graduation_service._llm", return_value=mock_llm), \
             patch("api.integrations.graduation_service._fetch_page", return_value=fake_page), \
             patch("api.integrations.graduation_service._is_valid_program_page", return_value=True):
    ...
    def test_returns_none_when_no_slug_matches(self):
        ...
        with patch("api.integrations.graduation_service._llm", return_value=mock_llm), \
             patch("api.integrations.graduation_service._fetch_page", return_value=None):
            result = _discover_url_via_slug(
...
class TestDiscoverCalendarUrlFallback:
    def test_slug_404_then_web_search_success(self):
        from api.integrations.graduation_service import _discover_calendar_url
        page = {
            "url": "https://utsc.calendar.utoronto.ca/computer-science-specialist",
            "text": "CSCA08H3 CSCA48H3 MATA30H3",
            "html": "<html></html>",
        }
        with patch("api.integrations.graduation_service._discover_url_via_slug", return_value=None), \
             patch("api.integrations.graduation_service._discover_url_via_web_search", return_value=page):
...
class TestAdditionalDiscoveryHelpers:
    def test_slugify_strips_symbols(self):
        from api.integrations.graduation_service import _slugify
    ...
    def test_find_base_specialist_url_returns_none_when_not_coop_slug(self):
        from api.integrations.graduation_service import _find_base_specialist_url
    ...
    def test_discover_url_via_web_search_uses_ddg_fallback(self):
        from api.integrations.graduation_service import _discover_url_via_web_search
        page = {"url": "https://utsc.calendar.utoronto.ca/cs", "text": "CSCA08H3", "html": ""}
        with patch("api.integrations.graduation_service._anthropic_web_search_url", return_value=None), \
             patch("api.integrations.graduation_service._ddg_search_urls", return_value=["https://utsc.calendar.utoronto.ca/cs"]), \
             patch("api.integrations.graduation_service._fetch_page", return_value=page):
    ...
    def test_anthropic_web_search_returns_none_on_exception(self):
        from api.integrations.graduation_service import _anthropic_web_search_url
...
class TestGraduationAdditionalHelpers:
    def test_db_and_llm_helpers_build_clients(self, monkeypatch):
        from api.integrations.graduation_service import _db, _llm
        monkeypatch.setenv("SUPABASE_URL", "https://example.supabase.co")
        monkeypatch.setenv("SUPABASE_KEY", "supabase-key")
        monkeypatch.setenv("ANTHROPIC_API_KEY", "anthropic-key")
        with patch("api.integrations.graduation_service.create_client", return_value="db-client") as mock_create, \
             patch("api.integrations.graduation_service.anthropic.Anthropic", return_value="llm-client") as mock_llm:
    ...
    def test_current_academic_year_branches(self):
        from api.integrations.graduation_service import _current_academic_year
        from datetime import datetime, timezone
        with patch("api.integrations.graduation_service.datetime") as mock_dt:
    ...
    def test_discover_calendar_url_uses_default_base_and_slug_success(self):
        ...
        with patch("api.integrations.graduation_service._discover_url_via_slug", return_value=page) as mock_slug:
    ...
    def test_get_program_requirements_uses_current_year_default(self):
        ...
        with patch("api.integrations.graduation_service._load_cache", return_value=None), \
             ...
             patch("api.integrations.graduation_service._save_cache") as mock_save:
    ...
    def test_find_base_specialist_url_probes_candidates_until_match(self):
        from api.integrations.graduation_service import _find_base_specialist_url
        with patch("api.integrations.graduation_service._fetch_page", side_effect=[
            None,
    ...
    def test_find_base_specialist_url_returns_none_after_exhausting_candidates(self):
        from api.integrations.graduation_service import _find_base_specialist_url
        with patch("api.integrations.graduation_service._fetch_page", return_value=None):
            result = _find_base_specialist_url(
    ...
    def test_discover_url_via_web_search_falls_back_after_404(self):
        from api.integrations.graduation_service import _discover_url_via_web_search
        page = {"url": "https://utsc.calendar.utoronto.ca/cs", "text": "CSCA08H3", "html": ""}
        with patch("api.integrations.graduation_service._anthropic_web_search_url", return_value="https://utsc.calendar.utoronto.ca/missing"), \
             patch("api.integrations.graduation_service._fetch_page", side_effect=[None, page]), \
             patch("api.integrations.graduation_service._ddg_search_urls", return_value=["https://utsc.calendar.utoronto.ca/cs"]):
            result = _discover_url_via_web_search("Computer Science Specialist", "https://utsc.calendar.utoronto.ca", False)
        assert result == page
    ...
    def test_discover_url_via_web_search_returns_page_from_anthropic_url(self):
        from api.integrations.graduation_service import _discover_url_via_web_search
        page = {"url": "https://utsc.calendar.utoronto.ca/cs", "text": "CSCA08H3", "html": ""}
        with patch("api.integrations.graduation_service._anthropic_web_search_url", return_value="https://utsc.calendar.utoronto.ca/cs"), \
             patch("api.integrations.graduation_service._fetch_page", return_value=page):
    ...
    def test_discover_url_via_web_search_returns_none_when_all_fallbacks_fail(self):
        from api.integrations.graduation_service import _discover_url_via_web_search
        with patch("api.integrations.graduation_service._anthropic_web_search_url", return_value=None), \
             patch("api.integrations.graduation_service._ddg_search_urls", return_value=[]):
    ...
    def test_discover_url_via_slug_swallows_llm_exceptions(self):
        from api.integrations.graduation_service import _discover_url_via_slug
        with patch("api.integrations.graduation_service._llm", side_effect=RuntimeError("boom")), \
             patch("api.integrations.graduation_service._fetch_page", return_value=None):
            assert _discover_url_via_slug("Program", "https://utsc.calendar.utoronto.ca", False, ["program"]) is None

    def test_campus_digit_and_course_credits_helpers(self):
        from api.integrations.graduation_service import _campus_digit, _course_credits
    ...
    def test_match_group_unknown_item_type_and_match_coop_statuses(self):
        from api.integrations.graduation_service import _match_group, _match_coop

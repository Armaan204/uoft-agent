...
class TestConfidenceHelpers:
    def test_confidence_exact_keyword(self):
        from integrations.syllabus import _confidence
    ...
    def test_score_candidate_picks_best(self):
        from integrations.syllabus import _score_candidate
    ...
    def test_allowed_ext_pdf(self):
        from integrations.syllabus import _allowed_ext
    ...
    def test_extract_page_slug_present(self):
        from integrations.syllabus import _extract_page_slug
...
class TestPickBestCandidate:
    def test_empty_returns_none(self):
        from integrations.syllabus import _pick_best_candidate
...
class TestCollectFileCandidates:
    def test_filters_allowed_extensions(self):
        from integrations.syllabus import _collect_file_candidates
    ...
    def test_debug_wrapper_returns_error_when_inner_raises(self):
        from integrations.syllabus import _collect_file_candidates_debug
        with patch("integrations.syllabus._collect_file_candidates", side_effect=RuntimeError("boom")):
...
class TestCollectModuleCandidates:
    def test_collects_file_items_from_modules(self):
        from integrations.syllabus import _collect_module_candidates
        mock_client = MagicMock()
    ...
    def test_debug_wrapper_returns_error_when_inner_raises(self):
        from integrations.syllabus import _collect_module_candidates_debug
        with patch("integrations.syllabus._collect_module_candidates", side_effect=RuntimeError("boom")):
...
class TestPageCandidateCollectors:
    def test_collect_syllabus_body_page_candidates(self):
        from integrations.syllabus import _collect_syllabus_body_page_candidates
        mock_client = MagicMock()
    ...
    def test_collect_module_page_candidates(self):
        from integrations.syllabus import _collect_module_page_candidates
        mock_client = MagicMock()
    ...
    def test_collect_frontpage_page_candidates(self):
        from integrations.syllabus import _collect_frontpage_page_candidates
        mock_client = MagicMock()
...
class TestFindSyllabusFile:
    def test_returns_none_when_no_candidates(self):
        from integrations.syllabus import find_syllabus_file
        mock_client = MagicMock()
        mock_client.get_course_files.return_value = []
        mock_client.get_course_modules.return_value = []
    ...
    def test_returns_best_url_immediately_when_top_confidence_exceeds_threshold(self):
        from integrations.syllabus import find_syllabus_file
        with patch("integrations.syllabus._collect_file_candidates", return_value=[{"name": "x", "url": "http://a/syllabus.pdf", "confidence": 1.0}]), \
             patch("integrations.syllabus._collect_module_candidates", return_value=[]):
    ...
    def test_low_confidence_asks_claude(self):
        ...
        with patch("integrations.syllabus._ask_claude_pick_syllabus", return_value="http://a/file.pdf") as mock_pick:
            url = find_syllabus_file(101, mock_client)
    ...
    def test_deduplicates_urls_and_returns_high_confidence_best(self):
        ...
        with patch("integrations.syllabus._collect_module_candidates", return_value=[{"name": "duplicate.pdf", "url": "http://a/syllabus.pdf", "confidence": 0.0}]):
...
class TestFindSyllabusFrontpage:
    def test_returns_none_when_no_front_page(self):
        from integrations.syllabus import find_syllabus_frontpage
        mock_client = MagicMock()
...
class TestFindSyllabusPage:
    def test_returns_none_when_no_candidates(self):
        from integrations.syllabus import find_syllabus_page
        mock_client = MagicMock()
    ...
    def test_returns_none_when_best_candidate_has_zero_confidence(self):
        from integrations.syllabus import find_syllabus_page
        with patch("integrations.syllabus._collect_syllabus_body_page_candidates", return_value=[{"page_slug": "x", "confidence": 0.0}]), \
             patch("integrations.syllabus._collect_module_page_candidates", return_value=[]), \
             patch("integrations.syllabus._collect_frontpage_page_candidates", return_value=[]):
            assert find_syllabus_page(101, MagicMock()) is None

    def test_skips_duplicate_slugs(self):
        from integrations.syllabus import find_syllabus_page
        dup = {"page_slug": "x", "confidence": 1.0}
        with patch("integrations.syllabus._collect_syllabus_body_page_candidates", return_value=[dup]), \
             patch("integrations.syllabus._collect_module_page_candidates", return_value=[dup]), \
             patch("integrations.syllabus._collect_frontpage_page_candidates", return_value=[]):
...
class TestDownloadPdf:
    def test_returns_bytes_on_success(self):
        from integrations.syllabus import _download_pdf
        fake_bytes = b"%PDF-1.4 test content"
        mock_resp = MagicMock(ok=True, content=fake_bytes)
        with patch("integrations.syllabus.requests.get", return_value=mock_resp):
            result = _download_pdf("http://a/file.pdf")
        assert result == fake_bytes

    def test_raises_on_http_error(self):
        from integrations.syllabus import _download_pdf, SyllabusError
...
class TestExtractText:
    def test_raises_on_empty_pdf(self):
        from integrations.syllabus import _extract_text, SyllabusError
        mock_reader = MagicMock()
        mock_reader.pages = [MagicMock(extract_text=lambda: "")]
        with patch("integrations.syllabus.PdfReader", return_value=mock_reader):
    ...
    def test_returns_text_from_pages(self):
        from integrations.syllabus import _extract_text
        page = MagicMock()
    ...
    def test_raises_syllabus_error_on_pypdf_exception(self):
        from integrations.syllabus import _extract_text, SyllabusError
        with patch("integrations.syllabus.PdfReader", side_effect=Exception("corrupt")):
...
class TestAskClaude:
    ...
    def test_returns_parsed_weights(self):
        from integrations.syllabus import _ask_claude
        fake_response = self._make_claude_response('{"Midterm": 40, "Final": 60}')
        mock_client = MagicMock()
        mock_client.messages.create.return_value = fake_response
        with patch("integrations.syllabus._get_anthropic_client", return_value=mock_client):
    ...
    def test_raises_on_invalid_json(self):
        from integrations.syllabus import _ask_claude, SyllabusError
...
class TestParseSyllabusWeights:
    def test_cache_hit_returns_cached(self):
        from integrations.syllabus import _parse_syllabus_weights_cached
        cached = {"Midterm": 40.0, "Final": 60.0}
        with patch("integrations.syllabus._load_persisted_weights", return_value=cached):
    ...
    def test_pdf_url_path_downloads_and_parses(self):
        ...
        with patch("integrations.syllabus._load_persisted_weights", return_value=None), \
             patch("integrations.syllabus._download_pdf", return_value=b"bytes") as mock_dl, \
             patch("integrations.syllabus._extract_text", return_value="text") as mock_ext, \
             patch("integrations.syllabus._ask_claude", return_value=expected_weights) as mock_ask, \
             patch("integrations.syllabus._save_persisted_weights"):
            source, weights = _parse_syllabus_weights_cached(
                101, "key", MagicMock(), pdf_url="http://a/file.pdf"
            )
    ...
    def test_no_source_raises_syllabus_error(self):
        from integrations.syllabus import _parse_syllabus_weights_cached, SyllabusError
        mock_client = MagicMock()
        with patch("integrations.syllabus.find_syllabus_file", return_value=None), \
             patch("integrations.syllabus.find_syllabus_frontpage", return_value=None), \
             patch("integrations.syllabus.find_syllabus_page", return_value=None):
    ...
    def test_syllabus_body_html_fallback(self):
        ...
        with patch("integrations.syllabus.find_syllabus_file", return_value=None), \
             patch("integrations.syllabus.find_syllabus_frontpage", return_value=None), \
             patch("integrations.syllabus.find_syllabus_page", return_value=None), \
             patch("integrations.syllabus._load_persisted_weights", return_value=None), \
             patch("integrations.syllabus._extract_text_from_html", return_value="text"), \
             patch("integrations.syllabus._ask_claude", return_value=expected_weights), \
             patch("integrations.syllabus._save_persisted_weights"):
            source, weights = _parse_syllabus_weights_cached(
                101, "key", mock_client,
    ...
    def test_canvas_page_fallback(self):
        ...
        with patch("integrations.syllabus.find_syllabus_file", return_value=None), \
             patch("integrations.syllabus.find_syllabus_frontpage", return_value=None), \
             patch("integrations.syllabus.find_syllabus_page", return_value=page_candidate), \
             patch("integrations.syllabus._load_persisted_weights", return_value=None), \
             patch("integrations.syllabus._extract_text_from_html", return_value="text"), \
             patch("integrations.syllabus._ask_claude", return_value=expected_weights), \
             patch("integrations.syllabus._save_persisted_weights"):
    ...
    def test_canvas_page_cache_hit(self):
        ...
        with patch("integrations.syllabus.find_syllabus_file", return_value=None), \
             patch("integrations.syllabus.find_syllabus_frontpage", return_value=None), \
             patch("integrations.syllabus.find_syllabus_page", return_value=page_candidate), \
             patch("integrations.syllabus._load_persisted_weights", return_value=cached):
            source, weights = _parse_syllabus_weights_cached(101, "key", mock_client)
        assert weights == cached
    ...
    def test_docx_source_flows_through_same_download_parse_pipeline(self):
        from integrations.syllabus import _parse_syllabus_weights_cached
        with patch("integrations.syllabus._load_persisted_weights", return_value=None), \
             patch("integrations.syllabus._download_pdf", return_value=b"docx-bytes") as mock_dl, \
             patch("integrations.syllabus._extract_text", return_value="Assignments 100%"), \
             patch("integrations.syllabus._ask_claude", return_value={"Assignments": 100}), \
             patch("integrations.syllabus._save_persisted_weights"):
            source, weights = _parse_syllabus_weights_cached(
...
class TestPersistedWeightsHelpers:
    def test_save_persisted_weights_swallows_cache_error(self):
        from integrations.syllabus import _save_persisted_weights
        from integrations.syllabus_cache import SyllabusCacheError
        with patch("integrations.syllabus.save_cached_syllabus_weights", side_effect=SyllabusCacheError("fail")):
            _save_persisted_weights(101, "src", {"Midterm": 40})

    def test_load_persisted_weights_returns_none_on_cache_error(self):
        from integrations.syllabus import _load_persisted_weights
        from integrations.syllabus_cache import SyllabusCacheError
        with patch("integrations.syllabus.get_cached_syllabus_weights", side_effect=SyllabusCacheError("fail")):
            assert _load_persisted_weights(101, "src") is None

    def test_get_anthropic_client_requires_key(self, monkeypatch):
        from integrations.syllabus import _get_anthropic_client, SyllabusError
...
class TestAskClaudePickSyllabus:
    ...
    def test_returns_matching_url(self):
        from integrations.syllabus import _ask_claude_pick_syllabus
        candidates = [
...
class TestSyllabusAdditional:
    def test_get_anthropic_client_builds_client(self, monkeypatch):
        from integrations.syllabus import _get_anthropic_client
        fake = object()
        monkeypatch.setenv("ANTHROPIC_API_KEY", "test-key")
        with patch("integrations.syllabus.anthropic.Anthropic", return_value=fake) as mock_ctor:
    ...
    def test_extract_text_from_html_raises_on_empty(self):
        from integrations.syllabus import _extract_text_from_html, SyllabusError
        with pytest.raises(SyllabusError, match="no extractable text"):
            _extract_text_from_html("<div>   </div>")

    def test_load_persisted_weights_returns_cached_value(self):
        from integrations.syllabus import _load_persisted_weights
        with patch("integrations.syllabus.get_cached_syllabus_weights", return_value={"Midterm": 40.0}):
    ...
    def test_parse_syllabus_weights_wrapper_uses_client_cache_key(self):
        ...
        with patch("integrations.syllabus._parse_syllabus_weights_cached", return_value=("src", {"Midterm": 40.0})) as mock_cached:
    ...
    def test_extract_text_from_html_success_and_save_persisted_weights_success(self):
        from integrations.syllabus import _extract_text_from_html, _save_persisted_weights
        assert _extract_text_from_html("<p>Midterm</p><p>40%</p>") == "Midterm\n40%"
        with patch("integrations.syllabus.save_cached_syllabus_weights") as mock_save:
    ...
    def test_parse_syllabus_weights_cached_syllabus_body_cache_hit(self):
        ...
        with patch("integrations.syllabus.find_syllabus_file", return_value=None), \
             patch("integrations.syllabus.find_syllabus_frontpage", return_value=None), \
             patch("integrations.syllabus.find_syllabus_page", return_value=None), \
             patch("integrations.syllabus._load_persisted_weights", return_value={"Midterm": 40.0}):

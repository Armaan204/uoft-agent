...
class TestExtractText:
    def test_single_text_block(self):
        from agent.agent import _extract_text
...
class TestRunEndTurn:
    ...
    def test_returns_plain_string_by_default(self):
        ...
        with patch("agent.agent.anthropic.Anthropic", mock_anthropic), \
             patch("agent.agent.QuercusClient"):
    ...
    def test_history_capped_at_max(self):
        from agent.agent import run
        from agent.agent import _MAX_HISTORY_MESSAGES
        content = [_make_text_block("Done")]
        mock_anthropic = MagicMock()
        mock_anthropic.return_value.messages.create.return_value = _make_response("end_turn", content)
...
class TestRunToolUse:
    def test_single_tool_call_then_end_turn(self):
        ...
        with patch("agent.agent.anthropic.Anthropic", mock_anthropic), \
             patch("agent.agent.QuercusClient"), \
             patch("agent.agent.execute_tool", return_value={"courses": []}):
    ...
    def test_tool_call_recorded_in_all_tool_calls(self):
        ...
        with patch("agent.agent.anthropic.Anthropic", mock_anthropic), \
             patch("agent.agent.QuercusClient"), \
             patch("agent.agent.execute_tool", return_value=tool_result):
    ...
    def test_verbose_prints_tool_info(self, capsys):
        ...
        with patch("agent.agent.anthropic.Anthropic", mock_anthropic), \
             patch("agent.agent.QuercusClient"), \
             patch("agent.agent.execute_tool", return_value={}):
            run("Q", verbose=True)
    ...
    def test_verbose_truncates_long_result(self, capsys):
        ...
        with patch("agent.agent.anthropic.Anthropic", mock_anthropic), \
             patch("agent.agent.QuercusClient"), \
             patch("agent.agent.execute_tool", return_value=big_result):
            run("Q", verbose=True)
        captured = capsys.readouterr()
    ...
    def test_skips_non_tool_use_blocks_in_tool_response(self):
        ...
        with patch("agent.agent.anthropic.Anthropic", mock_anthropic), \
             patch("agent.agent.QuercusClient"), \
             patch("agent.agent.execute_tool", return_value={}) as mock_exec:
    ...
    def test_tool_results_fed_back_to_claude(self):
        ...
        with patch("agent.agent.anthropic.Anthropic", mock_anthropic), \
             patch("agent.agent.QuercusClient"), \
             patch("agent.agent.execute_tool", return_value={"ok": True}):
            run("Q", verbose=False)

...
class TestGetCachedGrades:
    def test_returns_empty_when_no_snapshot(self):
        ...
        from agent.tools import _get_cached_grades
        client = _make_quercus_client()

        with patch(
            "agent.tools.load_grades_snapshot",
            return_value=[],
        ):
    ...
    def test_fetched_at_is_max_of_all_rows(self):
        ...
        with patch("agent.tools.load_grades_snapshot", return_value=rows):
...
class TestGetAllGrades:
    def test_calls_get_grade_snapshot_when_user_id_present(self):
        ...
        from agent.tools import _get_all_grades
        client = _make_quercus_client()
        expected = {"courses": [{"course_id": 999}], "errors": [], "fetched_at": "now"}

        with patch("agent.tools.get_grade_snapshot", return_value=expected) as mock_snap:
...
class TestGetAcademicHistory:
    def test_returns_error_without_user_id(self):
        from agent.tools import _get_academic_history
        client = _make_quercus_client()
    ...
    def test_returns_structured_acorn_data(self, sample_acorn_data):
        ...
        with patch("agent.tools.load_academic_history", return_value=expected_history):
    ...
    def test_delegated_to_acorn_service(self):
        ...
        with patch("agent.tools.load_academic_history", return_value={}) as mock_load:
    ...
    def test_returns_empty_history_payload(self):
        from agent.tools import _get_academic_history
        client = _make_quercus_client()
        with patch("agent.tools.load_academic_history", return_value={"terms": [], "credits_earned": 0.0}):
    ...
    def test_returns_multiple_terms(self):
        ...
        with patch("agent.tools.load_academic_history", return_value=payload):
            result = _get_academic_history({}, client, user_id="user-1")
...
class TestAnnouncementsTools:
    ...
    def test_get_announcement_detail_success(self):
        from agent.tools import _get_announcement_detail
        mock_client = MagicMock()
...
class TestGradeToolsHelpers:
    def test_get_course_weights_prefers_canvas(self):
        from agent.tools import _get_course_weights
        mock_client = MagicMock()
    ...
    def test_get_course_weights_falls_back_to_syllabus(self):
        ...
        with patch("agent.tools.parse_syllabus_weights", return_value=("src", {"Final": 100.0})):
            result = _get_course_weights({"course_id": 1}, mock_client)
        assert result == {"Final": 100.0}

    def test_preview_text_truncates(self):
        from agent.tools import _preview_text
        preview = _preview_text("<p>" + ("a" * 150) + "</p>", limit=20)
        assert preview.endswith("…")
        assert len(preview) == 20

    def test_get_current_grade_delegates_to_calculator(self):
        from agent.tools import _get_current_grade
        mock_client = MagicMock()
        mock_client.get_assignment_groups.return_value = [{"name": "Midterm", "assignments": []}]
        mock_client.get_submissions.return_value = []
        with patch("agent.tools._get_course_weights", return_value={"Midterm": 100.0}), \
             patch("agent.tools._calc.current_grade", return_value={"weighted_grade": 0.0}) as mock_calc:
    ...
    def test_build_grade_summary_maps_grade_fields(self):
        from agent.tools import _build_grade_summary
        course = {"id": 1, "name": "Intro CS", "course_code": "CSCA08H3"}
        with patch("agent.tools._get_current_grade", return_value={"weighted_grade": 80.0, "letter": "A-", "gpa_points": 3.7, "graded_weight": 60.0}):
    ...
    def test_get_all_grades_collects_partial_errors(self):
        ...
        with patch("agent.tools._build_grade_summary", side_effect=fake_summary):
            result = _get_all_grades({}, mock_client, user_id=None)
    ...
    def test_refresh_grades_without_user_id_falls_back_to_live_fetch(self):
        from agent.tools import _refresh_grades
        mock_client = MagicMock()
        with patch("agent.tools._get_all_grades", return_value={"courses": []}) as mock_all:
            result = _refresh_grades({}, mock_client, user_id=None)
        mock_all.assert_called_once()
        assert result == {"courses": []}

    def test_check_graduation_progress_without_user_id(self):
        from agent.tools import _check_graduation_progress
        result = _check_graduation_progress({}, MagicMock(), user_id=None)
        assert "error" in result

    def test_check_graduation_progress_blank_program_name(self):
        from agent.tools import _check_graduation_progress
        with patch("agent.tools.load_academic_history", return_value={"programs": [{"programName": "   "}]}):
    ...
    def test_get_grade_scenarios_returns_no_ungraded_error(self):
        from agent.tools import _get_grade_scenarios
        mock_client = MagicMock()
        ...
        with patch("agent.tools._get_course_weights", return_value={"Midterm": 100.0}), \
             patch("agent.tools._calc.current_grade", return_value={"weighted_grade": 80.0}):
    ...
    def test_get_grade_scenarios_success(self):
        ...
        mock_client.get_submissions.return_value = []
        with patch("agent.tools._get_course_weights", return_value={"Final Exam": 60.0}), \
             patch("agent.tools._calc.current_grade", return_value={"weighted_grade": 70.0}), \
             patch("agent.tools._calc.grade_scenarios", return_value={"A": {"status": "needed", "needed": 90.0}}):
            result = _get_grade_scenarios({"course_id": 1}, mock_client)
...
class TestCheckGraduationProgressTool:
    def test_returns_error_when_no_acorn_data(self):
        ...
        from agent.tools import _get_cached_grades  # ensure module loads

        mock_client = MagicMock()
        mock_client._token = "tok"

        with patch("agent.tools.load_academic_history", return_value={"terms": [], "credits_earned": 0.0}), \
             patch("agent.tools._get_prog_reqs", return_value=None):
            from agent.tools import execute_tool
    ...
    def test_returns_progress_dict_on_success(self, sample_requirements):
        ...
        with patch("agent.tools.load_academic_history", return_value=acorn_with_programs), \
             patch("agent.tools._get_prog_reqs", return_value=sample_requirements), \
             patch("agent.tools._check_grad_progress", return_value=expected) as mock_progress:
            result = execute_tool("check_graduation_progress", {}, mock_client, "user-99")
...
class TestRefreshGrades:
    def test_refresh_grades_invalidates_and_saves_snapshot(self):
        ...
        with patch("agent.tools.invalidate_grade_snapshot") as mock_invalidate, \
             patch("agent.tools.get_grade_snapshot", return_value=fresh) as mock_snapshot, \
             patch("agent.tools.save_snapshot") as mock_save:
...
class TestAgentToolsAdditional:
    ...
    def test_get_grade_scenarios_uses_substring_weight_match(self):
        ...
        mock_client.get_submissions.return_value = []
        with patch("agent.tools._get_course_weights", return_value={"Final": 55.0}), \
             patch("agent.tools._calc.current_grade", return_value={"weighted_grade": 72.0}), \
             patch("agent.tools._calc.grade_scenarios", return_value={"A": {"status": "needed", "needed": 90.0}}):
            result = _get_grade_scenarios({"course_id": 1}, mock_client)
    ...
    def test_get_grade_scenarios_uses_shortest_containing_candidate(self):
        ...
        mock_client.get_submissions.return_value = []
        with patch("agent.tools._get_course_weights", return_value={"final exam": 40.0, "final project showcase": 60.0}), \
             patch("agent.tools._calc.current_grade", return_value={"weighted_grade": 60.0}), \
             patch("agent.tools._calc.grade_scenarios", return_value={"B": {"status": "needed", "needed": 70.0}}):
            result = _get_grade_scenarios({"course_id": 1}, mock_client)

...
class TestNormaliseImportCode:
    def test_uppercases_and_strips(self):
        from api.integrations.acorn_store import _normalise_import_code
    ...
    def test_raises_when_all_special_chars(self):
        from api.integrations.acorn_store import _normalise_import_code, AcornStoreError
...
class TestWriteReadStatus:
    def test_write_then_read_roundtrip(self, tmp_path):
        from api.integrations.acorn_store import write_latest, read_latest, IMPORTS_DIR
        payload = {
            "importCode": "TESTCODE",
            "courses": [{"courseCode": "CSC490H1"}],
        }
        with patch("api.integrations.acorn_store.IMPORTS_DIR", tmp_path):
    ...
    def test_read_returns_none_when_no_file(self, tmp_path):
        from api.integrations.acorn_store import read_latest
        with patch("api.integrations.acorn_store.IMPORTS_DIR", tmp_path):
            result = read_latest("NOSUCHCODE")
        assert result is None

    def test_get_status_when_data_exists(self, tmp_path):
        from api.integrations.acorn_store import write_latest, get_status
        payload = {
            "importCode": "MYCODE",
    ...
    def test_get_status_when_no_data(self, tmp_path):
        from api.integrations.acorn_store import get_status
        with patch("api.integrations.acorn_store.IMPORTS_DIR", tmp_path):
    ...
    def test_path_for_code_uses_normalised_name(self, tmp_path):
        from api.integrations.acorn_store import _path_for_code
        with patch("api.integrations.acorn_store.IMPORTS_DIR", tmp_path):

...
class TestEncryptionImportFailureHandler:
    def test_module_raises_on_fernet_import_failure(self):
        ...
        import sys
        import importlib
        real_mod = sys.modules.pop("integrations.encryption", None)
        try:
            with patch.dict(sys.modules, {"cryptography.fernet": None}), \
                 patch("builtins.print"), \
                 patch("traceback.print_exc"):
                with pytest.raises(ImportError):
                    importlib.import_module("integrations.encryption")
        finally:
            sys.modules.pop("integrations.encryption", None)
            if real_mod is not None:
                sys.modules["integrations.encryption"] = real_mod


class TestGetCipher:
    def test_raises_when_key_not_set(self):
        from integrations.encryption import _get_cipher, EncryptionError
        with patch("integrations.encryption.os.getenv", return_value=None):
            with pytest.raises(EncryptionError, match="ENCRYPTION_KEY is not set"):
                _get_cipher()

    def test_raises_on_invalid_key(self):
        from integrations.encryption import _get_cipher, EncryptionError
        with patch("integrations.encryption.os.getenv", return_value="not-a-valid-fernet-key!"):
            with pytest.raises(EncryptionError, match="ENCRYPTION_KEY is invalid"):
                _get_cipher()

    def test_returns_cipher_with_valid_key(self):
        from integrations.encryption import _get_cipher
        from cryptography.fernet import Fernet
        with patch("integrations.encryption.os.getenv", return_value=_VALID_KEY):
            cipher = _get_cipher()
...
class TestEncryptToken:
    def test_raises_on_empty_string(self):
        from integrations.encryption import encrypt_token, EncryptionError
...
class TestDecryptToken:
    def test_raises_on_empty_string(self):
        from integrations.encryption import decrypt_token, EncryptionError
        with pytest.raises(EncryptionError, match="non-empty"):
            decrypt_token("")
...
class TestGetSyllabusCacheClient:
    def test_raises_when_url_and_key_missing(self):
        from integrations.syllabus_cache import _get_supabase_client, SyllabusCacheError
        with patch("integrations.syllabus_cache._secret_or_env", return_value=None):
    ...
    def test_raises_when_only_key_missing(self):
        ...
        with patch("integrations.syllabus_cache._secret_or_env", side_effect=_fake_env):
            with pytest.raises(SyllabusCacheError, match="SUPABASE_KEY"):
                _get_supabase_client()

    def test_returns_client_when_credentials_set(self):
        from integrations.syllabus_cache import _get_supabase_client
        mock_client = MagicMock()
        with patch("integrations.syllabus_cache.create_client", return_value=mock_client):
...
class TestSecretOrEnv:
    def test_returns_env_var_value(self):
        from integrations.syllabus_cache import _secret_or_env
...
class TestGetCachedSyllabusWeights:
    def test_returns_weights_on_cache_hit(self):
        from integrations.syllabus_cache import get_cached_syllabus_weights
        mock_sb = _mock_sb()
        mock_sb.table.return_value.execute.return_value = MagicMock(
            data=[{"weights": {"Midterm": 40.0, "Final": 60.0}}]
        )
        with patch("integrations.syllabus_cache._get_supabase_client", return_value=mock_sb):
    ...
    def test_raises_on_db_error(self):
        from integrations.syllabus_cache import get_cached_syllabus_weights, SyllabusCacheError
        mock_sb = MagicMock()
...
class TestSaveCachedSyllabusWeights:
    def test_inserts_when_no_existing_row(self):
        from integrations.syllabus_cache import save_cached_syllabus_weights
        mock_sb = _mock_sb()
    ...
    def test_raises_on_db_check_error(self):
        from integrations.syllabus_cache import save_cached_syllabus_weights, SyllabusCacheError
        mock_sb = MagicMock()
        mock_sb.table.side_effect = Exception("DB down")
        with patch("integrations.syllabus_cache._get_supabase_client", return_value=mock_sb):
